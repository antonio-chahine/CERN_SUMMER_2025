            ...
            for event in events:
                ...
                for hit in event.get('VertexBarrelCollection'):
                    try:
                        ...
                        module_hits[cellID].append((trackID, h, pid))

                    except Exception as e:
                        print(f"Skipping hit due to error: {e}")

                # Cluster hits per module
                for cellID, hit_group in module_hits.items():
                    particles = {}
                    for trackID, h, pid in hit_group:
                        key = (trackID, cellID)
                        if key not in particles:
                            particles[key] = functions.Particle(trackID=trackID, cellID=cellID, pid=pid)
...
if args.classify:
    ...
    with open('muons_edep.pkl', 'rb') as f:
        muons = pickle.load(f)
    with open('signal_edep.pkl', 'rb') as f:
    ...
    # === Select threshold that gives ≥99% signal retention with max bkg rejection
    target_tpr = 0.99
